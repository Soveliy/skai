...
const scripts = () => {
    gulp.src([
        srcFolder + '/assets/scripts/libraries/**/*.js',
    ])
    // .pipe(gulpBabel({
    //     presets: ['@babel/env'],
    // }))
    // .pipe(gulpUglify())
    .pipe(gulp.dest(distFolder + '/assets/scripts/libraries/'))
    gulp.src([
        srcFolder + '/assets/scripts/plugins/**/*.js',
    ])
    // .pipe(gulpBabel({
    //     presets: ['@babel/env'],
    // }))
    // .pipe(gulpUglify())
    .pipe(gulp.dest(distFolder + '/assets/scripts/plugins/'))
    return gulp.src([
...
const SVGIcons = () => {
    return gulp.src([
        srcFolder + '/assets/svg-sprite/svg-icons/*.svg',
    ])
    .pipe(gulpSVGO({
        plugins: [
            { removeXMLNS: true, },
    ...
    .pipe(gulp.dest(srcFolder + '/assets/svg-sprite/'))
    .pipe(browserSync.stream());
};
...
const watch = () => {
    gulp.watch([
        srcFolder + '/*.html',
        srcFolder + '/assets/templates/**/*.html',
    ], markup);
    gulp.watch([
        srcFolder + '/assets/styles/**/*.{css,scss}',
    ], styles);
    gulp.watch([
